...
def gen_pred_batches(dataset, batch_size=256):
  dataset, _ =  trim_dataset(dataset)
  sequences = flatten_dataset(dataset)
  ...
  for i in range(0, len(perm), batch_size):
    ...
    for j in batch_perm:
      seq = sequences[j]
      codes.append(seq['codes'][:-1])
      deltas.append(seq['deltas'][:-1])
      pred_codes.append(seq['codes'][-1])
      pred_deltas.append(seq['deltas'][-1])

    codes = np.array(codes)
    deltas = np.array(deltas)
    pred_codes = np.array(pred_codes)
